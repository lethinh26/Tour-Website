import { FileTextOutlined, ScheduleOutlined, SettingOutlined, PoweroffOutlined, DownOutlined, UserOutlined, HeartOutlined } from "@ant-design/icons";
    ...
    return (
        <div>
            ...
            >
                ...
                <span className="text-[#007AFF] font-bold text-sm tracking-wide ml-1">{user?.name || "User"}</span>

                <DownOutlined style={{ fontSize: 12 }} className={`text-[#007AFF] transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`} />
            </button>
                        ...
                        {menuItems.map((item, index) => {
                            ...
                            return (
                                <li key={item.id}>
                                    ...
                                    >
                                        <span className={newLocal}>{item.icon}</span>
                                        <span className="font-medium text-[15px] text-gray-700">{item.label}</span>
                                    </button>
                                </li>
                            );
                        })}
            ...
            {isOpen && (
                <div
                    className="w-screen h-screen fixed top-0 left-0 x-10"
                    onClick={() => {
                        setIsOpen(false);
                    }}
                ></div>
