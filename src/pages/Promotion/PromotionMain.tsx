...
import { Button, Pagination, notification, Tooltip } from "antd";
...
export default function PromotionMain() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (token) {
                    const userRes = await promotionAPI.getByToken(token);
                    setUserPromotions(userRes.data?.promotion || []);
                }
            } catch (error) {
    ...
    const pro = Array.isArray(promotions)
        ? promotions
              .filter((item) => !userPromotionIds.includes(item.id) && item.amount > 0)
              .map((item) => {
                  return {
                      id: item.id,
                      type: item.type,
    ...
    const handleSavePromotion = async (promotionId: number) => {
        ...
        if (!token) {
            api.error({
                ...
                description: "Vui lòng đăng nhập để lưu khuyến mãi",
                placement: "topRight",
            });
            return;
        }
        ...
        try {
            const res = await promotionAPI.addToUser(token, promotionId);
            console.log('addToUser response:', res);
            ...
            if (res.status === 200) {
                ...
                const [updatedRes, userRes] = await Promise.all([
                    promotionAPI.getAll(),
                    promotionAPI.getByToken(token)
                ]);
                
                setPromotions(updatedRes.data);
                setUserPromotions(userRes.data?.promotion || []);
            }
        ...
        <>
            ...
            <div className="min-h-screen bg-gray-50">
                <div className="bg-linear-to-r from-blue-500 to-blue-400 text-white px-[150px] pb-8 pt-2 rounded-b-3xl">
                    <h1 className="text-2xl font-bold mb-2">Mã giảm giá Triploka ở đây chứ đâu xa!</h1>
                                    ...
                                    >
                                        {token && (
                                            <div className="absolute top-3 right-2">
                                                <Tooltip title="Lưu khuyến mãi">
                                                    <button
                                                        className="text-xl bg-none hover:text-2xl cursor-pointer"
                                                        onClick={() => {
                                    ...
                                    <div className="p-4">
                                        <div className="text-sm text-gray-600 mb-2 flex items-center gap-2">
                                            ...
                                            {promo.endAt ? (
                                                ...
                                                <span className="font-medium">Bắt đầu từ {formatDateToString(promo.startAt)}</span>
                                            )}
                                        </div>
                                        ...
                                        <div className="flex items-center justify-between gap-2">
                                            <div className="flex items-center gap-2 flex-1">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <div className="font-mono text-sm font-semibold text-gray-800">{promo.code}</div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="text-xs font-bold text-gray-500 mt-4">
                                            Số lượng còn lại: <span className="inline text-red-400">{promo.amount}</span>
                                        </div>
                                    </div>
                                </div>
                            ))
                ...
                <div className="flex justify-center items-center gap-2 py-6">
                    <Pagination align="center" defaultCurrent={currentPage} total={pro.length} onChange={setCurrentPage} pageSize={pageSize} />
                </div>
            </div>
        </>
    );
